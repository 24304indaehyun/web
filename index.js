        ...
        function autoProduction() {
            ...
            if (canWork) {
                ...
                let waterMultiplier_disaster = 1; // 가뭄 패널티
        ...
        function updateUI() {
            ...
            // --- 1. 요청 사항: 유휴 인구가 마이너스가 되지 않도록 함 (이 계산은 UI 표시용이며, 실제 로직은 재해 함수에서 처리)
            ...
            if (gameState.activeEffects.droughtActive) {
                droughtVisual.innerHTML = `<span class="plague-indicator" title="가뭄이 지속되고 있습니다">☀️</span>`; // plague-indicator의 깜박임 재활용
            } else {
        ...
        function showMessage(message, type = 'info') {
            ...
            } else if (type === 'success') { // --- 3. 긍정적 이벤트 타입 추가
                messageElement.style.backgroundColor = '#28a745'; // 녹색
                messageElement.style.color = 'white';
            } else { // info
                messageElement.style.backgroundColor = '#17a2b8'; // 청록색
                messageElement.style.color = 'white';
            }
        ...
        function endWildfireEffect() {
            ...
            // --- 1. 요청 사항: 타이머 ID가 남아있을 경우(수동 진화로 종료된 경우)를 대비해 초기화 ---
            if(gameState.activeEffects.wildfireTimer) {
                clearTimeout(gameState.activeEffects.wildfireTimer);
        ...
        function endDroughtEffect() {
            ...
            updateUI(); // --- 2. 요청 사항: 가뭄 종료 시 UI 즉시 업데이트 (이모지 제거) ---
        }
            ...
            },
            {
                ...
                effect: (start) => {
                    ...
                    if (currentLevel > 1) {
                        ...
                        let workersRemoved = 0; // --- 1. 유휴 인구 전환 카운터 ---
                        ...
                        // --- 1. 알림 메시지에 해제된 일꾼 수 포함 ---
                        const alertText = `🚨 지진 발생! 🏘️${buildingToHurt} 건물의 레벨이 1 하락했습니다! ${workersRemoved > 0 ? `(인구 수용량 초과로 ${workersRemoved}명이 유휴 인구가 됩니다)` : ''}`;
            ...
            },
            {
                ...
                effect: (start) => {
                    ...
                    gameState.activeEffects.wildfireTimer = setTimeout(() => {
                        if (gameState.activeDisaster === 'wildfire') {
                            endWildfireEffect();
                            gameState.activeEffects.wildfireTimer = null; // 타이머 만료 후 ID 초기화
                        }
            ...
            },
            {
                ...
                effect: (start) => {
                    ...
                    if (workersToKill > 0) {
                        ...
                        // --- 2. 요청 사항: 신규 인구 로직 적용 ---
                        // (예: 3레벨(12명) -> 1명 사망(11명), 레벨 유지. / 3레벨(12명) -> 7명 사망(5명), 2레벨(6명)로 하락)
                        ...
                        // --- 1. 역병으로 인한 레벨 하락 시에도 인구 과잉 확인 ---
                        const newPopulation = gameState.buildings.house.population; // 이미 사망자가 반영된 인구
                        let totalAssigned = Object.values(gameState.workers).reduce((sum, count) => sum + count, 0) + gameState.buildings.restaurant.workers;
                        let overflow = totalAssigned - newPopulation;
                        let workersUnassigned = 0; // 해제된 일꾼 수
                        ...
                        while (overflow > 0) {
                            ...
                            if (!unassignedThisCycle) break; // 더 이상 해제할 일꾼이 없음
                        }
                        
                        const alertText = `🚨 역병 발생! 총 ${totalDeaths}명의 인구가 사망했습니다! (총 인구 ${newPopulation}명) ${workersUnassigned > 0 ? `(인구 감소로 ${workersUnassigned}명이 유휴 상태가 됩니다)` : ''}`;
                        showAlert(alertText, 'error');
        ...
        const blessings = [
            {
                ...
                effect: () => {
                    ...
                    gameState.activeEffects.blessingActive.multiplier_food = 2; // 식량 생산 2배

                    const alertText = `🎉 풍작의 달! 🌕 20초간 🌾식량 생산량이 2배가 됩니다!`;
                    showAlert(alertText, 'success'); // 'success' 타입 사용 (녹색)
                    updateUI();
            ...
            },
            {
                ...
                effect: () => {
                    ...
                    gameState.activeEffects.blessingActive.multiplier_water = 2; // 물 생산 2배

                    const alertText = `🎉 단비가 내립니다! 🌧️ 20초간 💧물 생산량이 2배가 됩니다!`;
        ...
        function endBlessingEffect() {
            if (gameState.activeBlessing) {
                showAlert(`${gameState.activeBlessing === 'harvest_moon' ? '풍작' : '단비'} 효과가 종료되었습니다.`, 'info');
            }
        ...
        function startDisasterTimer() {
            ...
            if (!disasterTimer) {
                 disasterTimer = setInterval(rollForEvent, DISASTER_INTERVAL); // 함수 이름 변경
            }
        }
        
        // 재해 및 긍정적 이벤트 무작위 선택 및 실행
        function rollForEvent() { // 함수 이름 변경
            if (gameState.gameFinished || gameState.activeDisaster || gameState.activeBlessing) return; // 이미 재해 또는 축복이 발생 중이면 건너뜀
        ...
        function activateDisaster(disaster) {
            ...
            if (gameState.activeDisaster || gameState.activeBlessing) {
                showMessage(`[DEV] 이미 ${gameState.activeDisaster || gameState.activeBlessing} 이벤트가 활성화되어 있습니다.`, 'warning');
                return;
            }
        ...
        function showAlert(message, type) {
            ...
            if (type === 'warning') {
                ...
                alertElement.style.backgroundColor = '#28a745'; // 녹색
                alertElement.style.color = 'white';
            } else { // info
                alertElement.style.backgroundColor = '#17a2b8'; // 청록색
                alertElement.style.color = 'white';
            }
        ...
        function executeDevCommand() {
            ...
            if (parts.length !== 2) {
                showMessage('잘못된 형식입니다. (자원이름): (갯수) 또는 (disaster|blessing): (타입) 형식으로 입력하세요.', 'error');
                return;
            }
            ...
            if (commandType === 'disaster') {
                ...
                if (gameState.activeDisaster || gameState.activeBlessing) { // 3. 중복 체크
                    showMessage(`이미 ${gameState.activeDisaster || gameState.activeBlessing} 이벤트가 활성화되어 있습니다.`, 'warning');
                    return;
                }
            ...
            if (commandType === 'blessing') {
                ...
                if (!blessingObj) {
                    showMessage(`존재하지 않는 축복 타입입니다: ${blessingType} (가능: harvest_moon, gentle_rain)`, 'error');
                    return;
                }
                ...
                showMessage(`[DEV] 축복 ${blessingObj.name}를 강제 발생시켰습니다!`, 'success');
                return;
            }
            // --- 추가 끝 ---
